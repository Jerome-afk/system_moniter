...
void SystemInfo::drawPerformanceGraph(const char* label, std::vector<float>& history, float currentValue, 
    ...
    if (ImGui::Checkbox("Pause", &paused)) {
        // Reset update time when unpausing
        if (!paused) {
            if (label == std::string("CPU")) lastCpuUpdateTime = std::chrono::steady_clock::now();
            else if (label == std::string("Fan")) lastFanUpdateTime = std::chrono::steady_clock::now();
            else if (label == std::string("Thermal")) lastThermalUpdateTime = std::chrono::steady_clock::now();
        }
    }
    ...
    // Scale slider
    ImGui::SetNextItemWidth(120);
    ImGui::SliderFloat("Scale", &scale, 10.0f, 200.0f, "%.1f");
    
    ImGui::EndGroup();
    
    // Performance graph
    ImGui::PlotLines("##graph", history.data(), history.size(), 0, overlay, 0.0f, scale, 
...
void SystemInfo::drawSystemMonitor() {
    ...
    ImGui::Text("OS: %s", osType.c_str());
    ImGui::Text("User: %s", user.c_str());
    ImGui::Text("Hostname: %s", hostname.c_str());
    ImGui::Text("CPU: %s", cpuType.c_str());
    
    ImGui::Separator();
    
    ImGui::Text("Total Tasks: %d (Running: %d, Sleeping: %d, Uninterruptible: %d, Zombie: %d, Traced/Stopped: %d, Interrupted: %d)",
               tasks.total, tasks.running, tasks.sleeping, tasks.uninterruptible, 
               tasks.zombie, tasks.traced_stopped, tasks.interrupted);
    
    ImGui::EndChild();
    
    // CPU, Fan and Thermal section with tabs
    ImGui::BeginChild("PerformanceGraphs", ImVec2(0, 0), true);
    
    ImGui::BeginTabBar("SystemTabs");
    
    // CPU tab
    if (ImGui::BeginTabItem("CPU")) {
        char overlay[32];
        snprintf(overlay, sizeof(overlay), "%.1f%%", cpuData.usagePercent);
        drawPerformanceGraph("CPU", cpuData.history, cpuData.usagePercent, 
                          cpuData.paused, cpuData.fps, cpuData.scale, overlay);
        ImGui::EndTabItem();
    }
    
    // Fan tab
    if (ImGui::BeginTabItem("Fan")) {
        ImGui::Text("Fan Status: %s", fanData.enabled ? "Enabled" : "Disabled");
        ImGui::Text("Speed: %d RPM", fanData.speed);
        ImGui::Text("Level: %d", fanData.level);
        
        char overlay[32];
        snprintf(overlay, sizeof(overlay), "%d RPM", fanData.speed);
        drawPerformanceGraph("Fan", fanData.history, fanData.speed, 
                          fanData.paused, fanData.fps, fanData.scale, overlay);
        ImGui::EndTabItem();
    }
    ...
    if (ImGui::BeginTabItem("Thermal")) {
        char overlay[32];
        snprintf(overlay, sizeof(overlay), "%.1f°C", thermalData.temperature);
        drawPerformanceGraph("Thermal", thermalData.history, thermalData.temperature, 
                          thermalData.paused, thermalData.fps, thermalData.scale, overlay);
        ImGui::EndTabItem();
    }
